"use client";

import { useEffect, useState } from "react";
import AccountMenu from "@/components/AccountMenu";
import BrandIcon from "@/components/BrandIcon";
import GameQuestionnaire from "@/components/GameQuestionnaire";
import Questionnaire from "@/components/Questionnaire";
import RiskReport from "@/components/RiskReport";
import SiteFooter from "@/components/SiteFooter";
import Spinner from "@/components/Spinner";
import StoreHeader from "@/components/StoreHeader";
import TrackSwitcher from "@/components/TrackSwitcher";
import { getTrack } from "@/lib/tracks";
import type { Answers, Diagnosis, GameAnswers, Track } from "@/lib/types";

type Status = "idle" | "loading" | "report" | "error";

const EMPTY_APP: Answers = {
  buildTool: "",
  dataPractices: [],
  safariDiff: "",
  downloadsCode: "",
  webViewShell: "",
  nativeFeatures: [],
  codePaste: "",
};

const EMPTY_GAME: GameAnswers = {
  buildTool: "",
  existingIP: [],
  originality: "",
  monetization: [],
  gambling: "",
  audienceData: [],
  codePaste: "",
};

// The whole checker, parameterized by track. The app (light) and game (dark)
// routes both render this; only the questionnaire, copy, theme, and price differ.
export default function CheckerExperience({ track }: { track: Track }) {
  const cfg = getTrack(track);
  const empty: Answers | GameAnswers = track === "game" ? EMPTY_GAME : EMPTY_APP;

  const [status, setStatus] = useState<Status>("idle");
  const [answers, setAnswers] = useState<Answers | GameAnswers>(empty);
  const [diagnosis, setDiagnosis] = useState<Diagnosis | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  // Remember which section you're in so the shared account pages (My reports,
  // login) follow its theme: app = light, game = dark.
  useEffect(() => {
    document.cookie = `vc_track=${track};path=/;max-age=2592000;samesite=lax`;
  }, [track]);

  async function runCheck() {
    setStatus("loading");
    setErrorMsg(null);
    try {
      const res = await fetch("/api/check", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ answers, track }),
      });
      const data: unknown = await res.json().catch(() => null);
      if (!res.ok || !data || typeof data !== "object" || !("riskLevel" in data)) {
        const message =
          data && typeof data === "object" && "error" in data
            ? String((data as { error: unknown }).error)
            : null;
        setErrorMsg(message);
        setStatus("error");
        return;
      }
      setDiagnosis(data as Diagnosis);
      setStatus("report");
    } catch {
      setErrorMsg(null);
      setStatus("error");
    }
  }

  function reset() {
    setAnswers(empty);
    setDiagnosis(null);
    setErrorMsg(null);
    setStatus("idle");
  }

  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-6 sm:px-8 sm:py-14">
      <div className="mb-8 flex items-center justify-between gap-3">
        {status !== "idle" ? (
          <div className="flex items-center gap-2.5">
            <BrandIcon track={track} size="sm" />
            <span className="text-sm font-semibold tracking-tight text-ink">
              VibeCheck
            </span>
          </div>
        ) : (
          <span aria-hidden />
        )}
        <div className="flex items-center gap-2.5">
          <TrackSwitcher active={track} />
          <AccountMenu />
        </div>
      </div>

      {status === "idle" && (
        <div className="space-y-6 sm:space-y-8">
          <StoreHeader track={track} />
          <section className="space-y-4 sm:space-y-5">
            <div>
              <p className="font-display text-xs uppercase tracking-[0.2em] text-ink-muted">
                Pre-submission review
              </p>
              <h2 className="mt-1.5 text-lg font-bold tracking-tight text-ink">
                Tell us about your {cfg.noun}
              </h2>
            </div>
            {track === "game" ? (
              <GameQuestionnaire
                value={answers as GameAnswers}
                onChange={(next) => setAnswers(next)}
                onSubmit={runCheck}
              />
            ) : (
              <Questionnaire
                value={answers as Answers}
                onChange={(next) => setAnswers(next)}
                onSubmit={runCheck}
              />
            )}
          </section>
        </div>
      )}

      {status === "loading" && (
        <div className="flex flex-col items-center justify-center rounded-[var(--radius-card)] border border-line bg-surface px-6 py-20 text-center shadow-card">
          <Spinner className="h-7 w-7" />
          <p className="mt-5 text-base font-medium text-ink">
            Submitting your {cfg.noun} for review…
          </p>
          <p className="mt-2 font-display text-xs uppercase tracking-[0.18em] text-ink-faint">
            Checking against Apple&rsquo;s guidelines
          </p>
        </div>
      )}

      {status === "report" && diagnosis && (
        <RiskReport diagnosis={diagnosis} answers={answers} onReset={reset} />
      )}

      {status === "error" && (
        <div className="rounded-[var(--radius-card)] border border-line bg-surface px-6 py-16 text-center shadow-card">
          <p className="text-base font-medium text-ink">
            Couldn&rsquo;t finish the review.
          </p>
          <p className="mx-auto mt-2 max-w-sm text-sm text-ink-muted">
            {errorMsg ??
              `We couldn't analyze your ${cfg.noun} just now. Your answers are still here, so give it another try.`}
          </p>
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <button
              type="button"
              onClick={runCheck}
              className="w-full rounded-full bg-accent px-6 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90 sm:w-auto"
            >
              Try again
            </button>
            <button
              type="button"
              onClick={reset}
              className="w-full rounded-full border border-line-strong bg-surface px-6 py-3 text-sm font-medium text-ink transition-colors hover:bg-surface-2 sm:w-auto"
            >
              Start over
            </button>
          </div>
        </div>
      )}

      <SiteFooter />
    </main>
  );
}
